...
function display_with_auto_contrast() { //Make the stack better displayed (increase contrast for 405 and 488 images) without changing the raw data
    ...
    for (i = 1; i < nSlices + 1; i++) { //Iterate all slices
        ...
        if (i <= 2) { //Skip the last slice, which is the bright-field
            ...
            // run("Apply LUT", "slice"); //Only apply the contrast adjustment to that slice rather than the whole stack. I commented this out since later I found out that clicking on the Apply will change the raw data on the image. I have tested out and this line is why your code's threshold is different from Dave's code
        }
    }
}
